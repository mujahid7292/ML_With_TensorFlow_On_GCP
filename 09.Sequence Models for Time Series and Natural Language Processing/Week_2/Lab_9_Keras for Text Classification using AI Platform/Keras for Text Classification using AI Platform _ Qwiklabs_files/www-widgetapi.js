...
;new J;function Bh(a){if(!a)throw Error();throw a;}
...
V.all=function(a){return new V(function(b,c){var d=[],e=a.length;0===e&&b(d);for(var f={D:0};f.D<a.length;f={D:f.D},++f.D)Dh(V.resolve(a[f.D]).then(function(g){return function(h){d[g.D]=h;e--;0===e&&b(d)}}(f)),function(g){c(g)})})};
V.resolve=function(a){return new V(function(b,c){a instanceof V?a.then(b,c):b(a)})};
V.reject=function(a){return new V(function(b,c){c(a)})};
V.prototype.then=function(a,b){var c=this,d=null!==a&&void 0!==a?a:Ch,e=null!==b&&void 0!==b?b:Bh;return new V(function(f,g){"PENDING"===c.state.status?(c.f.push(function(){Eh(c,c,d,f,g)}),c.onRejected.push(function(){Fh(c,c,e,f,g)})):"FULFILLED"===c.state.status?Eh(c,c,d,f,g):"REJECTED"===c.state.status&&Fh(c,c,e,f,g)})};
...
function Ih(a){return new K(function(b,c){Hh(a,b,c)})}
function W(a){return new V(function(b,c){Hh(a,b,c)})}
;function Jh(a,b){return new V(function(c,d){function e(){var f=a?b(a):null;f?f.then(function(g){a=g;e()},d):c()}
e()})}
;function Kh(a,b){for(var c=[],d=1;d<arguments.length;++d)c[d-1]=arguments[d];d=Error.call(this,a);this.message=d.message;"stack"in d&&(this.stack=d.stack);this.args=[].concat(c instanceof Array?c:ia(u(c)))}
ra(Kh,Error);var Lh=[];function Mh(a,b){Lh.push({type:"EVENT",eventType:a,payload:b});10<Lh.length&&Lh.shift()}
;var Nh={},Oh=(Nh.AUTH_INVALID="No user identifier specified.",Nh.EXPLICIT_ABORT="Transaction was explicitly aborted.",Nh.IDB_NOT_SUPPORTED="IndexedDB is not supported.",Nh.MISSING_OBJECT_STORE="Object store not created.",Nh.UNKNOWN_ABORT="Transaction was aborted for unknown reasons.",Nh.QUOTA_EXCEEDED="The current transaction exceeded its quota limitations.",Nh.QUOTA_MAYBE_EXCEEDED="The current transaction may have failed because of exceeding quota limitations.",Nh);
function Ph(a,b,c){b=void 0===b?{}:b;c=void 0===c?Oh[a]:c;Kh.call(this,c,Object.assign({name:"YtIdbKnownError",isSw:void 0===self.document,isIframe:self!==self.top,type:a},b));this.type=a;this.message=c;Object.setPrototypeOf(this,Ph.prototype);Lh.push({type:"ERROR",payload:this});10<Lh.length&&Lh.shift()}
ra(Ph,Kh);function Qh(a,b,c){Ph.call(this,"UNKNOWN_ABORT",{objectStoreNames:a,dbName:b,mode:c});Object.setPrototypeOf(this,Qh.prototype)}
...
e instanceof Qh&&(Mh("TRANSACTION_UNEXPECTEDLY_ABORTED",{objectStoreNames:c,transactionDuration:f-d,transactionCount:a.transactionCount,dbDuration:f-a.h}),a.f=!0);Xh(a,!1,c,f-d)})}
function Xh(a,b,c,d){Mh("TRANSACTION_ENDED",{objectStoreNames:c,connectionHasUnknownAbortedTransaction:a.f,duration:d,isSuccessful:b})}
...
function $h(a,b,c){a=a.f.openCursor(b.query,b.direction);return bi(a).then(function(d){return Jh(d,c)})}
...
b.f.addEventListener("abort",function(){var e=b.f.error;if(e){var f=ci(b.f.objectStoreNames).join(),g=b.f.db.name,h=b.f.mode,k;"QuotaExceededError"===e.name?k=new Ph("QUOTA_EXCEEDED",{objectStoreNames:f,dbName:g,mode:h}):"UnknownError"===e.name&&(k=new Ph("QUOTA_MAYBE_EXCEEDED",{objectStoreNames:f,dbName:g,mode:h}));k&&Mh("QUOTA_EXCEEDED",void 0);d(e)}else b.aborted||(e=new Qh(ci(b.f.objectStoreNames).join(),b.f.db.name,b.f.mode),d(e))})})}
function Vh(a,b){var c=new K(function(d,e){Dh(b(a).then(function(f){a.commit();d(f)}),e)});
...
return Ih(c).then(function(l){e&&l.addEventListener("versionchange",function(){e(b())});
...
Da=E.lineNumber||"Not available",Ri=E.fileName||"Not available",Si=E.stack||v.f||"Not available";if(v.hasOwnProperty("args")&&v.args&&v.args.length)for(var oa=0,Vb=0;Vb<v.args.length;Vb++){var P=v.args[Vb],za="params."+Vb;oa+=za.length;if(P)if(Array.isArray(P)){for(var Ti=w,gd=oa,jb=0;jb<P.length&&!(P[jb]&&(gd+=Fi(jb,P[jb],za,Ti),500<gd));jb++);oa=gd}else if("object"===typeof P){var Wb=void 0,Ui=w,hd=oa;for(Wb in P)if(P[Wb]&&(hd+=Fi(Wb,P[Wb],za,Ui),500<hd))break;oa=hd}else w[za]=Gi(P),oa+=w[za].length;
else w[za]=Gi(P),oa+=w[za].length;if(500<=oa)break}else if(v.hasOwnProperty("params")&&v.params){var Xb=v.params;if("object"===typeof v.params){var Jf=0;for(q in Xb)if(Xb[q]){var Kf="params."+q,Lf=Gi(Xb[q]);w[Kf]=Lf;Jf+=Kf.length+Lf.length;if(500<Jf)break}}else w.params=Gi(Xb)}navigator.vendor&&!w.hasOwnProperty("vendor")&&(w.vendor=navigator.vendor);var id={message:Ta,name:ha,lineNumber:Da,fileName:Ri,stack:Si,params:w},Mf=Number(v.columnNumber);isNaN(Mf)||(id.lineNumber=id.lineNumber+":"+Mf);for(var Z,
Aa=id,Nf=u(yi),jd=Nf.next();!jd.done;jd=Nf.next()){var kd=jd.value;if(kd.U[Aa.name])for(var Of=u(kd.U[Aa.name]),ld=Of.next();!ld.done;ld=Of.next()){var Pf=ld.value,Yb=Aa.message.match(Pf.regexp);if(Yb){Aa.params["error.original"]=Yb[0];for(var md=Pf.groups,Qf={},Ma=0;Ma<md.length;Ma++)Qf[md[Ma]]=Yb[Ma+1],Aa.params["error."+md[Ma]]=Yb[Ma+1];Aa.message=kd.Z(Qf);break}}}Z=Aa;window.yterr&&"function"===typeof window.yterr&&window.yterr(Z);if(!(Ci.has(Z.message)||0<=Z.stack.indexOf("/YouTubeCenter.js")||
0<=Z.stack.indexOf("/mytube.js"))){Bi.K("handleError",Z);if(R("kevlar_gel_error_routing")){var Zb=void 0,Rf=m,L=Z;a:{for(var Sf=u(Ei),nd=Sf.next();!nd.done;nd=Sf.next()){var Tf=F;if(Tf&&0<=Tf.toLowerCase().indexOf(nd.value.toLowerCase())){var Uf=!0;break a}}Uf=!1}if(!Uf){var kb={stackTrace:L.stack};L.fileName&&(kb.filename=L.fileName);var aa=L.lineNumber&&L.lineNumber.split?L.lineNumber.split(":"):[];0!==aa.length&&(1!==aa.length||isNaN(Number(aa[0]))?2!==aa.length||isNaN(Number(aa[0]))||isNaN(Number(aa[1]))||
(kb.lineNumber=Number(aa[0]),kb.columnNumber=Number(aa[1])):kb.lineNumber=Number(aa[0]));Ai||(Ai=new zi);var $b=Ai;var Vi=L.message,Wi=L.name;a:{for(var Vf=u($b.g),od=Vf.next();!od.done;od=Vf.next()){var Wf=od.value;if(L.message&&L.message.match(Wf.f)){var pd=Wf.weight;break a}}for(var Xf=u($b.f),qd=Xf.next();!qd.done;qd=Xf.next()){var Yf=qd.value;if(Yf.f(L)){pd=Yf.weight;break a}}pd=1}var rd={level:"ERROR_LEVEL_UNKNOWN",message:Vi,errorClassName:Wi,sampleWeight:pd};"ERROR"===Rf?rd.level="ERROR_LEVEL_ERROR":
"WARNING"===Rf&&(rd.level="ERROR_LEVEL_WARNNING");var Xi={isObfuscated:!0,browserStackInfo:kb},Na={pageUrl:window.location.href};N("FEXP_EXPERIMENTS")&&(Na.experimentIds=N("FEXP_EXPERIMENTS"));Na.kvPairs=[{key:"client.errorServiceSignature",value:"msg="+$b.g.length+"&cb="+$b.f.length}];var sd=L.params;if(sd)for(var Zf=u(Object.keys(sd)),td=Zf.next();!td.done;td=Zf.next()){var $f=td.value;Na.kvPairs.push({key:"client."+$f,value:String(sd[$f])})}var ag=N("SERVER_NAME",void 0),bg=N("SERVER_VERSION",
void 0);ag&&bg&&(Na.kvPairs.push({key:"server.name",value:ag}),Na.kvPairs.push({key:"server.version",value:bg}));var Yi={errorMetadata:Na,stackTrace:Xi,logMessage:rd};Zb=void 0===Zb?{}:Zb;var cg=xi;N("ytLoggingEventsDefaultDisabled",!1)&&xi==xi&&(cg=null);var Zi=cg,U=Zb;U=void 0===U?{}:U;var lb={};lb.eventTimeMs=Math.round(U.timestamp||T());lb.clientError=Yi;var $i=String;if(U.timestamp)var dg=-1;else{var eg=z("_lact",window);dg=null==eg?-1:Math.max(Date.now()-eg,0)}lb.context={lastActivityMs:$i(dg)};
if(R("log_sequence_info_on_gel_web")&&U.fa){var aj=lb.context,mb=U.fa;Xg[mb]=mb in Xg?Xg[mb]+1:0;aj.sequence={index:Xg[mb],groupKey:mb};U.Oa&&delete Xg[U.fa]}var bj=lb,ac=U.Na,fg=Zi,ud="";if(ac){var bc=ac,vd={};bc.videoId?vd.videoId=bc.videoId:bc.playlistId&&(vd.playlistId=bc.playlistId);Og[ac.token]=vd;ud=ac.token}var wd=Ng.get(ud)||[];Ng.set(ud,wd);wd.push(bj);fg&&(Ig=new fg);var cj=wg("web_logging_max_batch")||100,gg=T();wd.length>=cj?Pg(!0):10<=gg-Lg&&(Rg(),Lg=gg);Pg()}}if(!R("suppress_error_204_logging")){var xd,
