...
def read_and_preprocess(image_bytes, label=None, augment=False):
    # Decode the image, end up with pixel values, that are in 
    # the -1 To 1 range.
...
def serving_input_fn():
    # Note: Only handle one image at a time.
...
def make_input_fn(csv_of_filenames, batch_size, mode, augment=False):
    def _input_fn():
        ...
        if augment:
            dataset = dataset.map(
                map_func=read_and_preprocess_with_augment
            )
        else:
            dataset = dataset.map(
                map_func=read_and_preprocess
            )

        if mode == tf.estimator.ModeKeys.TRAIN
            num_epochs = None # Run Indefenitely
...
def image_classifier(input_tensor, hparams):
    ...
    # Get functions pointer for selected model type.
...
def model_fn(features, label, mode, params):
    ...
    logits = model._graph.get_tensor_by_name(
    ...
    class_int = tf.cast(
        x = tf.argmax(input=logits, axis=1),
        dtype = tf.uint8
    )
    class_str = tf.gather(
        params = LIST_OF_LABELS,
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or \
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            # This block of code is needed for Batch Normalization,
...
def train_and_evaluate(output_dir, hparams):
    # Ensure FileWriterCache is clear for tensorboard events file.
    ...
    # Set estimator's train_spec to use train_input_fn and
    # train for so many steps.
    ...
    # Create exporter that uses serving_input_fn() to
    # create save_model for serving.
    exporter = tf.estimator.LatestExporter(
        name="exporter",
        serving_input_receiver_fn=serving_input_fn
    )

    # Set estimator's eval_spec to use eval_input_fn() and
    # export saved_model.
